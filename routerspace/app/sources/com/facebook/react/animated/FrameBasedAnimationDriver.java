...
class FrameBasedAnimationDriver extends AnimationDriver {
    ...
    public void resetConfig(ReadableMap config) {
        ...
        if (config.hasKey("toValue")) {
            if (config.getType("toValue") == ReadableType.Number) {
                d = config.getDouble("toValue");
            }
            this.mToValue = d;
        } else {
            this.mToValue = 0.0d;
        ...
        if (config.hasKey("iterations")) {
            this.mIterations = config.getType("iterations") == ReadableType.Number ? config.getInt("iterations") : 1;
        } else {
            this.mIterations = 1;
        }
        this.mCurrentLoop = 1;
    ...
    public void runAnimationStep(long frameTimeNanos) {
        ...
        int round = (int) Math.round(((frameTimeNanos - this.mStartFrameTimeNanos) / 1000000) / FRAME_TIME_MILLIS);
        if (round < 0) {
