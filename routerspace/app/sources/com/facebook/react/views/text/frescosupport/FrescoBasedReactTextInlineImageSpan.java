...
public class FrescoBasedReactTextInlineImageSpan extends TextInlineImageSpan {
    ...
    public FrescoBasedReactTextInlineImageSpan(Resources resources, int height, int width, int tintColor, Uri uri, ReadableMap headers, AbstractDraweeControllerBuilder draweeControllerBuilder, Object callerContext, String resizeMode) {
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        canvas.save();
        canvas.translate(x, ((y + ((int) paint.descent())) - (((int) (paint.descent() - paint.ascent())) / 2)) - ((this.mDrawable.getBounds().bottom - this.mDrawable.getBounds().top) / 2));
